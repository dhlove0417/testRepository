...
public class BoardService {
	...
	public int insertBoard(Board b, Attachment at) {
		...
		//게시글 정보 등록부터 하기 (첨부파일은 해당 게시글 번호를 갖고 추가되어야하기 떄문에)
		...
		if(result*result2>0) { //성공시 (두 dml 다 0이 아닌경우)
			JDBCTemplate.commit(conn);
	...
	public int updateBoard(Board b, Attachment at) {
		...
		if(at!=null) {
			...
			if(at.getFileNo() !=0) {
				...
				//refBno(참조게시글번호) 부분이 currval로 들어가 있어서 사용할 수 없다.
	...
	public int insertPhotoBoard(Board b, ArrayList<Attachment> list) {
		...
		//둘중 하나라도 실패하면 rollback 둘다 성고애야 commit
		if(result*result2>0) {
			JDBCTemplate.commit(conn);
		}else {
			JDBCTemplate.rollback(conn);
