...
public class PhotoBoardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			//form에서 input hidden으로 로그인 유저 번호 전달하기
			...
			//각 데이터를 담아주기 위해 list준비
			ArrayList<Attachment> list = new ArrayList<>();
			
			//각 키 값을 반복 돌리면서 요소 꺼내주기
			
			for(int i=1; i<=4; i++) {
				//키값
				String key = "file"+i;
				
				//키 값에 해당하는 요소가 있는지 확인하기
				if(multipartRequest.getOriginalFileName(key)!=null) {
					//해당 키값에 파일이 있다면
					//Attachment객체 생성 후 데이터 담아주기
					//여러개가 나올 수 있으니 Attachment객체를 list에 추가하기
					//원본명,변경이름,파일경로,파일레벨(썸네일/상세사진)
