...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request.etAtribute(이름,데이터) 이름 내마음대루 작성
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			//webapp 뒤에 오는 경로는 따로 지정을 해야한다.
			//request.getSession().getServletContext():웹애플리케이션 객체를 의미
			...
			//setter메소드니까 순서 상관 없음
			Board b = new Board();
			b.setBoardContet(content);
			b.setBoardTitle(title);
			b.setBoardWriter(boardWriter);
			b.setCategory(category);
			
			//사요자가 파일을 업로드했다면 파일 정보를 추출하고 업로드하지 않았다면 게시글정보만 필요
