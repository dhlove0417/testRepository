...
public class BoardListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//listCount 현재 게시글 개수
		listCount = new BoardService().listCount(); //게시글 개수 조회해오기 //System.out.println(listCount);
		//현재페이지 정보(번호)
		 ...
		 * 
		 * 게시글 개수가 107개이고 한페이지에 10개씩 보여준다면
		 * 10페이지까지는 가득 차고 나머지 한 페이지에 남은 7개 게시글을 보여줘야한다
		 * 때문에 총 11개 페이지가 나오게됨
		 * 총개수/한페이지개수의 나머지를 올림처리하면 된다
		 * listCount/boardLimit ceil()
		 * 
		 * listCount와 boardLimit은 int자료형이기 때문에 
		 * 소수점 올림 처리를 하려면 둘중 하나는 double 변경하여
		 * 계산해야 나머지값이 나온다 (실수형) 처리 후 int로 변환하여 소수점 날리고 maxPage에 담기
		 * */
		maxPage=(int)Math.ceil((double)listCount/boardLimit); //둘다 int형이라 절삭당함 그래서 올림처리해도 무의미
