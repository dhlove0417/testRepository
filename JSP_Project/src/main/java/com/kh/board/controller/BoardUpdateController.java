...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//이떄 MultipartRequest 객체를 생성하는 구문이 업로드 처리를 하는 구문이기 때문에
		//해당 파일에 대한 설정을 해줘야함
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			//MultipartRequest객체 만들기 (객체가 생성되며 서버에 파일 업로드가 된다)
			...
			//기존첨부파일이 있을 때 새로운 첨부파일이 들어온경우(기존첨부파일 삭제헤주기)
			boolean flag = false;
			...
			if(multiRequest.getOriginalFileName("reUploadFile") != null) {
				...
				if(multiRequest.getParameter("originFileNo")!= null) {
					...
					//기존 첨부파일 삭제 플래그
					flag = true;
				}else { //새로 전달된 파일은 있지만 기존에는 파일이 없었을 때
