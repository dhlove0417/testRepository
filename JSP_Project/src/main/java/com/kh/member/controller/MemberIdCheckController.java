...
public class MemberIdCheckController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//해당 아이디값을 데이터베이스에서 있는지 없는지 조회처리
		int count = new MemberService().checkId(checkId); //해당 아이디가 존재하는지 숫자를 세어 있다면 사용 못하게 없다면 사용 가능하게
		
		//count에는 아이디가 있으면 (중복) 1 없으면 (사요가능) 0이 반환되었을것
