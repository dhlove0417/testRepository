	...
	<div class="outer">
		...
		<form action="${contextPath}/insert.ph" method="post" id="enroll-area" encType="multipart/form-data">
			...
			<div id="file-area">
                <!--onchange : 변화가 일어났을때 발생하는 이벤트
                    선언적함수를 내부에 작성할 때 해당 이벤트가 발생한 시점에 요소 객체를 전달하는 방법
                    함수(this)
                -->
        ...
        <script>
            $(function(){
                ...
                //대표이미지를 클릭하면 input file 요소 1번이 클릭디게 하는 함수
            ...
            function loadImg(inputFile,num){
                ...
                //파일을 선택하면 file속성의 length가 1이 반환됨
                if(inputFile.files.length==1){ //파일이 등록되면
                    //해당 파일을 읽어줄 FileReader라고 하는 js객체를 이용
                    var reader = new FileReader();
                    //파일을 읽어줄 메소드 :reader.readAsDataURL(파일)
                    //해당 파일을 읽어서 고유한 url을 부여해주는 메소드
                    //반환받은 url을 미리보기 화면에 넣어주면된다.
                    reader.readAsDataURL(inputFile.files[0]);

                    //해당 reader객체가 읽혀진 시점에 img src 속성에 부여된 고유 url을 넣어주기
                    reader.onload = function(e){ //e:
                        //이벤트 객체에서 reader가 부여한 고유 url정보
                        //event.target.result
                        switch(num){
